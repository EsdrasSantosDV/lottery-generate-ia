-- Tabelas de sync no `public` (schema padrão da API Supabase; sem expor schema extra).
...
-- Políticas para `anon` (VITE_SUPABASE_ANON_KEY / publishable). Ajuste em produção se usar auth.
